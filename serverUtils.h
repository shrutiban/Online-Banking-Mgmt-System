...
void viewDetails(int sd, struct database db)
{
	...
	// struct trans t1;
	// struct trans t2;
	// int flog = open("log.dat",O_RDWR , 0666);
    // int temp=open("templog.dat",O_CREAT|O_RDWR,0777);

	// while(read(flog, (char *)&t1, sizeof (struct trans)))
	// {
	// if(!strcmp(t1.account, db.acc_id)
	// 	{ write("templog.dat",(char *)&t1,sizeof(struct trans))
	// 		lseek(flog,-sizeof(struct trans),SEEK_CUR);
	// 	}
	// }
	// close(flog);
	// close(temp);
...
void deposit(int sd, struct database db)
{
    ...
    int flog=open("log.dat", O_CREAT | O_APPEND | O_RDWR , 0666);
	if(flog<0)
	{
	...
	t.date=__DATE__;
	t.time=__TIME__;  
    t.account=db.acc_id;// pass the account number  
    t.amount=amount;// amount that needs to be credit and debit 
    t.action='D'; //D for deposit and W withdraw 
	db.balance = db.balance + amount;
	t.famount=db.balance;  
	write(flog,&t,sizeof(struct trans)); // writing the log
	close(flog);
    findDetails(db);
	write(sd, &db.balance, sizeof(db.balance));
