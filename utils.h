...
void viewDetails(int sd)
{
    ...
    printf("\nDate-time\tAccount_Num\tAction\tAmount\tBalance\n");
 //    struct trans t1;
	// int flog = open("log.dat",O_RDWR , 0666);
    
	// while(read(flog, (char *)&t1, sizeof (struct trans))!=0)
	// {
	// if(t1.account==acc_id)
	// 	{ printf("\n%s-%s\t%d\t%c\t%d\t%d\n",t1.date,t1.time,t1.account,t1.action,t1.amount,t1.famount);
